    ...
    >
      ...
      {isMobile ? (
        ...
        <div className="relative isolate overflow-hidden">
              ...
              {(loading ? skeletonItems : categories).map((it, index) => {
                if (loading) {
                  return (
                    <div
                      key={`sk-${index}`}
                      className="w-[120px] md:w-32 lg:w-[132px]
                               h-40 md:h-[172px] lg:h-[180px]
                ...
                return (
                  ...
                  >
                    <div className="relative mx-2 mt-2 aspect-square overflow-hidden rounded-lg bg-gray-50 flex items-center justify-center">
                      {cat.images?.[0] ? (
                        <Image
                          src={cat.images[0]}
                          alt={cat.title}
                          ...
                          sizes="(max-width: 768px) 80px, (max-width: 1024px) 138px, 145px"
                        />
                      ) : (
                        <ShoppingBag className="text-[#167389]" size={24} />
                      )}
                    </div>
                    <p className="px-2 pb-2 pt-1 text-center text-sm font-semibold leading-tight text-gray-700 line-clamp-2">
                      {cat.title}
                    </p>
                  </Link>
                );
          ...
          {!loading && categories.length > 0 && (
            <>
              <div className="cat-fade-left pointer-events-none absolute inset-y-0 left-0 w-10 bg-linear-to-r from-white to-transparent" />
              <div className="cat-fade-right pointer-events-none absolute inset-y-0 right-0 w-10 bg-linear-to-l from-white to-transparent" />
            </>
          )}
        </div>
      )}
