...
export default async function CategoryView({ slug, isSubcategory = false }: CategoryViewProps) {
  if (isSubcategory) {
    ...
    const [subcategoriesRes, productsRes] = await Promise.all([
      ...
      fetchProducts({ subcategory: slug, limit: 20 }).catch(() => ({
        ok: true,
        data: [] as Product[],
      })),
    ]);
    ...
    // images[0] = thumbnail, images[1] = banner (fallback to images[0])
    const bannerImg = (activeSubcategory as any)?.images?.[1] ?? (activeSubcategory as any)?.images?.[0];
    const banners = bannerImg ? [{ _id: (activeSubcategory as any)?._id, title, image: bannerImg }] : [];
  ...
  const [categoriesRes, productsRes] = await Promise.all([
    ...
    fetchProducts({ category: slug, limit: 20 }).catch(() => ({
      ok: true,
      data: [] as Product[],
    })),
  ]);
  ...
  // images[0] = thumbnail, images[1] = banner (fallback to images[0])
  const bannerImg = (activeCategory as any)?.images?.[1] ?? (activeCategory as any)?.images?.[0];
  const banners = bannerImg ? [{ _id: activeCategory?._id, title, image: bannerImg }] : [];
              ...
              <div className="flex gap-3 pb-1">
                {subcategories.map((sub: any) => (
                  <Link
                    ...
                    className="min-w-[120px] shrink-0 rounded-xl border border-gray-200 bg-white transition hover:border-cyan-300 hover:shadow overflow-hidden"
                  >
