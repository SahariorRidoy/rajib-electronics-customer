...
export default function ProfilePage() {
  ...
  if (!isHydrated || (!isAuthed && isHydrated)) {
    return (
      <div className="min-h-screen bg-linear-to-br from-pink-50 to-rose-100 py-20 sm:py-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Left card skeleton */}
            <div className="lg:col-span-1 bg-white rounded-2xl border border-pink-100 shadow-sm p-5 space-y-4">
              <div className="flex items-center gap-3">
              ...
              <div className="space-y-4 pt-2">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex gap-2">
                    <div className="w-4 h-4 mt-1 bg-gray-200 animate-pulse rounded" />
            ...
            {/* Right card skeleton */}
            <div className="lg:col-span-2 bg-white rounded-2xl border border-pink-100 shadow-sm p-5 space-y-4">
              <div className="h-5 w-28 bg-gray-200 animate-pulse rounded" />
              <div className="h-3 w-64 bg-gray-100 animate-pulse rounded" />
              <div className="flex gap-2">
                <div className="flex-1 h-10 bg-gray-100 animate-pulse rounded-xl" />
                <div className="w-28 h-10 bg-gray-200 animate-pulse rounded-xl" />
              </div>
              ...
              {!loading &&
                orders.slice(0, 3).map((o) => (
                  <OrderCard
                    key={o._id}
                    order={o}
                    onViewInvoice={(order) => router.push(`/invoices/guest/${order._id}`)}
                  />
                ))}
            </div>
