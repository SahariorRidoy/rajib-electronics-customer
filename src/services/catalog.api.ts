...
export const catalogApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    /* ---------- Side Banners ---------- */
    getSideBanners: builder.query<Banner[], void>({
      async queryFn(_arg, _api, _extra, _baseQuery) {
        try {
          const res = await fetchBanners({ position: "side", status: "ACTIVE" });
      ...
      providesTags: [{ type: "Banner" as const, id: "SIDE" }],
    ...
    getProductById: builder.query<Product, string>({
      ...
      transformResponse: (response: { ok: boolean; data: Product }) => response.data,
